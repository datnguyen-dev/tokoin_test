...
func TestUserInitData(t *testing.T) {
	usr := user{contend: []byte(ByUser)}
	bol, _ := usr.InitData()
	assert.True(t, bol)
	assert.True(t, usr.mapJSONField != nil)
	assert.True(t, usr.mapJSONType != nil)
	assert.True(t, usr.mapValueSearch != nil)
}

func TestUserSearchField(t *testing.T) {
	usr := user{contend: []byte(ByUser)}
	usr.InitData()
	res, err := usr.GetSearchFields()
...
func TestUserSearch(t *testing.T) {
	...
	res, err := usr.Search("_id", "2")
	assert.True(t, len(res) > 0 && err == nil)
	res, err = usr.Search("_id", "")
	assert.True(t, err != nil)
	//test with locale
	res, err = usr.Search("locale", "de-CH")
	assert.True(t, len(res) > 0 && err == nil)
	res, err = usr.Search("locale", "")
	assert.True(t, err != nil)
	//test with tag
	res, err = usr.Search("tags", "Babb")
	assert.True(t, len(res) > 0 && err == nil)
	res, err = usr.Search("tags", "abdsdf")
	assert.True(t, err != nil)
}
